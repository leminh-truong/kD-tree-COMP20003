...
* an external data file and after performing nearest neighbor search for the
* queried point, outputs the information of the nearest neighbor of the queried
...
int main(int argc, char* argv[]){
	...
	x = (char*)malloc(BUFFER_SIZE*sizeof(x));
	assert(x != NULL);
	y = (char*)malloc(BUFFER_SIZE*sizeof(y));
	assert(y != NULL);
	...
	* KD - Tree with. */
	...
	while(fgets(trading_name, MAX_CHAR+1, stdin) != NULL){
		...
		/* Perform nearest neighbor search for queried point. */
		if(sscanf(trading_name,"%s %s", x, y) == 2){
			search_tree(company_tree, x, y, &comparisons, output_file);
		}
		printf("%s %s --> %d\n", x, y, comparisons);
	}
